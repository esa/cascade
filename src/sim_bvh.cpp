...
void sim::construct_bvh_trees_parallel()
{
    ...
    if (m_data->srt_mcodes.size()
        ...
        throw std::overflow_error(detail::overflow_err_msg);
    }
...
void sim::construct_bvh_trees_serial(unsigned chunk_idx)
{
            ...
            [&](const auto &rn, std::uint32_t init) {
                ...
                for (auto node_idx = rn.begin(); node_idx != rn.end(); ++node_idx) {
                    ...
                    if (cur_node.end - cur_node.begin > 1u && cur_node.split_idx <= 63) {
                        ...
                        split_ptr
                            = std::lower_bound(mcodes_begin, mcodes_end, 1u,
...
void sim::verify_bvh_trees_parallel() const
{
    oneapi::tbb::parallel_for(oneapi::tbb::blocked_range(0u, m_data->nchunks), [&](const auto &range) {
        for (auto chunk_idx = range.begin(); chunk_idx != range.end(); ++chunk_idx) {
            verify_bvh_trees_serial(chunk_idx);
        }
    });
}

void sim::verify_bvh_trees_serial(unsigned chunk_idx) const
{
    assert(chunk_idx < m_data->nchunks);
    ...
    for (decltype(bvh_tree.size()) i = 0; i < bvh_tree.size(); ++i) {
        ...
        if (cur_node.left != -1) {
            ...
            assert(detail::first_diff_bit(mcodes_ptr[split_idx], mcodes_ptr[split_idx + 1u]) == cur_node.split_idx);
